...
### 근무자 키워드 편집 페이지 (`WorkerKeywordPage.jsx`)
- 근무자 모드 전용 라우트 `/worker-keywords` — 근무자 로그인 시에만 접근(`App.jsx` `isWorker` 가드)
- 공익/재난 키워드 3개 항목만 로드/표시하고 `getFieldsValue([...3키])`로 그 3개만 `PUT /api/settings` 저장
- 설정 저장 API는 IP 허용 외 별도 인증이 없어 근무자도 호출 가능(백엔드 무변경)
...
- **폴더 감시와 상호 배타**: 예약 시각에 `_is_watcher_enabled()`가 참이면 FTP 가져오기를 건너뛰고,
  대신 `refresh_fetch_status(전날)`로 파일 존재 여부를 점검해 미완이면 `daily_fetch='missing'`(달력 붉은색)으로 기록
- **누락 판정 공용 함수** `refresh_fetch_status(date)`: 로컬 폴더(apst_dir/ddr1_dir/cml_path)에서
  APST/DDR1/CML 3종 파일 존재 여부(`_present_files_for_date`)를 검사 → 모두 있으면 `ok`,
  하나라도 없으면 `missing`(메시지에 없는 파일 종류 표기). FTP `fetch_and_ingest`·폴더감시 스케줄러
  분기·감시기 `_ingest_apst` 후처리가 모두 이 함수로 통일(기존 APST 단독 판정 대체)
- **감시 시작 초기 스캔**: `start_watching`이 `_initial_scan`(CML→APST→DDR1 순, 처리분 건너뜀)을
  백그라운드 스레드로 실행 → 감시 시작 전 이미 존재하던 파일도 적재되고 붉은색 자동 해제
- 감시기 적재 시 붉은색 자동 해제: `_ingest_apst`가 적재 후 `refresh_fetch_status(날짜)` 재평가

### 폴더 감시 — 파일 도착 순서 무관 재적재 (`_reconcile_date`)
- 문제: 감시는 파일마다 개별 이벤트라 순서가 보장되지 않음. 수동송출은 APST 적재 시점에
  DDR1 로그를 함께 읽어 생성되므로, APST가 DDR1/CML보다 먼저 처리되면 수동송출이 누락됨.
  (앱 내 FTP 가져오기는 `ingest_date()`로 CML→APST→DDR1 순서를 보장해 정상)
- 해결: 감시 3핸들러(`_ingest_apst/_ingest_ddr1/_ingest_cml`)가 각자 파일 처리 후 공통
  `_reconcile_date(date)` → `ingest_date(date)`(순서 보장 + 중복검사) + `refresh_fetch_status(date)` 호출.
  어떤 파일이 먼저/나중에 도착해도 그 시점에 전체가 재적재되어 수동송출까지 생성되고 붉은색이 해제됨.
- 부수 효과: 기존 감시 전용 DDR1 로그 탐색이 `.log`(소문자)를 못 찾던 버그도, `ingest_date`가
  `*.Log`+`*.log`를 모두 검색하므로 우회됨.

### 로그 노이즈 / FTP 미접근 환경(EC2) 대응
- `_insert_manual_segments`(ingest.py): DDR1 로그 미발견 시 `file_missing` 경고를 남기지 않고 조용히
  건너뜀. 감시 순서상 일시적이며, 실제 누락은 `refresh_fetch_status`(3파일 점검) 붉은색으로 표시됨.
- 스케줄러 감시-스킵 분기: FTP를 건너뛸 때 로그를 남기지 않음(조용히 전날 적재 점검만).
- `routers/ftp.py`: **폴더 감시 모드면 `/ftp/fetch`·`/fetch-yesterday`가 FTP 다운로드 대신
  `_reingest_local`(ingest_date + refresh_fetch_status)** 실행. EC2처럼 회사 FTP에 접근 불가한 환경에서
  붉은날짜 클릭 시 timeout 오류 없이 로컬 파일(ftp_to_ec2.sh로 push된)로 재적재·붉은색 해제.
- 달력 붉은색 산출(`get_missing_dates`)은 `status IN ('missing','error')` — 파일 없음·네트워크 오류 모두 포함
...
- 추가: `ftp_host/port/user/password/fetch_time`, `allowed_ip_ranges`,
  ...
  `company_name`(기본 광주문화방송), `company_short`(기본 광주MBC)
- 제거: `report_dir`(미사용)
- 시딩 전용 기본값(비워도 복구 안 함): 공익/재난 키워드 3종 + 회사명·약칭 — `init_db`의 `_seed_defaults`
- `ADMIN_PASSWORD_DEFAULT`: `admin` → `admin2450`(신규 빈 DB에만 적용)

### 회사명·약칭 표기 (`company_name` / `company_short`)
- 약칭(`company_short`): 좌측 로고(`App.jsx`), 브라우저 탭 제목(`document.title`), 월 리포트 제목
- 회사명(`company_name`): 월 리포트 info 테이블 '회사명' 칸, 하단 「○○(주)」
- `generate_monthly_pdf`/`generate_monthly_docx`가 `settings`에서 읽어 사용(빈 값이면 광주문화방송/광주MBC 폴백)
- 프론트는 앱 로드 시 `getSettings()`로 약칭을 받아 로고·`document.title`에 반영(`index.html` 기본값 `SB 송출 관리`)

### 소재별 월 리포트 조회·저장 일치 (단일 매칭 버그)
- `MonthlyTab`: 검색 결과가 1개면 모달 없이 바로 조회하되, **`selectItem(정확한 소재명)`으로 `item` 상태를 갱신**해야 함.
  (과거엔 `doReport`만 호출해 `item`이 입력한 부분 문자열로 남아, `?item=`을 쓰는 PDF/Word 저장이 빈 문서가 됨)
- PDF/Word 저장 URL은 `item`(=정확 소재명), 화면 조회도 같은 이름 → 일치

### 월 리포트 TV 시간 칸 표기 (PDF/Word 공통)
- PDF(`generate_monthly_pdf`): TV 시간 문자열을 `_wrap_cell`(Paragraph)로 감싸 칸 넘칠 때 자동 줄바꿈(행 높이 자동 확장)
- Word(`generate_monthly_docx`): TV 칸을 가운데 정렬 + 셀 세로 가운데(`WD_ALIGN_VERTICAL.CENTER`)로 PDF와 통일

### 로그 초기화 / 폴더 감시 유지
- `DELETE /api/logs` → `activity_log.clear_logs()`. 로그 화면 버튼은 `isAdmin`일 때만 표시(App가 `LogPage`에 `isAdmin` 전달)
- 폴더 감시 재시작 유지: 종료 훅은 `stop_watching(persist=False)`(설정 플래그 보존),
  시작 훅은 `auto_start_if_enabled()`(watcher_enabled='1'이면 자동 재개)
- 감시 초기 스캔은 `_scan_running` 가드로 중복 실행 방지, CML은 `reconcile=False`로 clip_map만 갱신(활동로그 미기록)

### Ubuntu(사내 서버) 배포
- `init_ubuntu.sh`(OS 셋팅) → `setup_ubuntu.sh`(apt 기반 앱 설치, setup_ec2의 포팅) → `install_service.sh`(systemd)
- 데이터 이전: `wsl_to_ubuntu_rsync.sh`(비밀번호 SSH + ControlMaster로 1회 입력)
- `install_service.sh`는 systemd라 Amazon Linux/Ubuntu 공통 동작

### 월 리포트 문서 레이아웃 — 여백 균형 + 1페이지 자동 맞춤
- **PDF**(`generate_monthly_pdf`): 상·하 여백 12mm 동일. `doc.build` 직전에 표를 제외한 요소 높이를 1회 측정하고, **측정 전용 복제 표**로 셀 상하 패딩을 6.0pt→1.4pt까지 낮추며 한 페이지에 들어오는 최대 패딩을 채택(넘침 방지 안전여유 포함). 실제 표 객체는 마지막에 한 번만 패딩 적용(측정 중 손상 방지).
- **Word**(`generate_monthly_docx`): 여백 12mm 동일. 렌더 높이를 직접 못 재므로 `_estimate_wrap_lines`(글자 수 기반 줄바꿈 추정)로 행 높이를 계산해 페이지를 채움(오버헤드 보수적 산정으로 2페이지 방지). 정보표·데이터표·푸터를 `WD_TABLE_ALIGNMENT.CENTER`로 가운데 정렬(좌우 여백 동일), 푸터는 테두리 없는 2칸 표(좌: 확인문구 10pt, 우: 회사명 11pt)로 한 줄 배치.
- **시간 표기**: `pdf_generator._to_broadcast_hhmm` / `docx_generator._hhmm`에서 `h<5 → +24` 변환 제거 → 화면과 동일하게 실제 시각 `00:xx` 출력.

### 수동송출 소재명 보완 — 양방향 APST 조회 + 백필
- `aggregator.get_apst_name_before(clip_id, before_date)`: ① 이전 날짜 중 최신 → ② 없으면 **이후 날짜 중 최초** 순으로 apst.db 조회(신소재 수동 선행 대응). clip_id↔소재명은 시점 무관 고정이라 미래 값 사용 안전.
- `ddr1_parser.extract_manual_segment_records`의 소재명 결정 순서: 당일 CML → 당일 APST DB → 전날 CML → **양방향 APST 조회(`apst_lookup`)** → 그래도 없으면 clip_id 유지. `ingest_date`가 `apst_lookup`을 넘겨 연결(파일감시의 `_ingest_manual_segments`는 미사용 死코드).
- `backfill_manual_names.py`: 기존 ddr1.db에서 `item_name = clip_id`(미확인 N-코드) 행을 소스 재적재 없이 재해석해 일괄 보완(`--apply`).

### APST 파일명 접미사 매칭 (`apst_suffix`, 대소문자 무시)
- `parsers/utils.apst_name_matches(filename, suffix, date=None)` / `find_apst_files(dir, suffix, date=None)`: 파일명 `<YYYYMMDD><suffix>.apst` 를 대소문자 무시로 매칭(확장자·접미사 모두). suffix 빈 값이면 `<YYYYMMDD>.apst`.
- 설정키 `apst_suffix`(기본 `AAA`, `_seed_defaults`) + `SettingsUpdate`/환경설정 UI 항목 추가.
- APST를 날짜로 찾는 모든 지점(ftp_fetcher `_present_files_for_date`, file_watcher 초기스캔·수동구간, ingest `ingest_date`·전체스캔, report 파일조회)을 이 헬퍼로 교체. 각 호출부는 설정값이 비면 `APST_SUFFIX_DEFAULT`로 폴백.

### 달력 상태(붉은/파랑) 판정 — `refresh_fetch_status`
- 붉은색(missing) 조건: ① 필수 파일(APST·DDR1 로그) 부재, 또는 ② `_unresolved_manual_clips(date)`(ddr1.db에서 `item_name = clip_id`인 미확인 수동송출)가 존재. **CML 부재는 붉은색 사유 아님**(메시지에만 참고 표기).
- 전체 스캔(`scan_apst_dir`)이 끝나면 처리한 날짜마다 `refresh_fetch_status` 호출로 상태 갱신(기존엔 미갱신).
- CML 폴더 감시: `start_watching`이 `_cml_watch_dir()`(cml_path가 파일이면 상위 폴더)를 `_CmlHandler`로 추가 감시 → 늦게 도착한 CML도 `_ingest_cml`→`refresh_fetch_status` 트리거(APST/DDR1와 중복 폴더면 등록 생략).
